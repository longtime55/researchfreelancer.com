<template>
    ...
    <div class="wt-accordioninnertitle">
            <div v-if="this.stored_project_img" :id="this.main_accordion_id" class="wt-projecttitle collapsed" data-toggle="collapse" :data-target="'#'+this.inner_accordion_id">
                ...
                <h3>{{this.stored_project_title}}<span>{{this.stored_project_date}}</span></h3>
            </div>
        ...
        <div class="wt-collapseexp collapse hide" :id="this.inner_accordion_id" :aria-labelledby="this.main_accordion_id" data-parent="#accordion">
            <fieldset>
                <div class="form-group form-group-half">
                    <input type="text" :value="this.stored_project_title" v-bind:name="this.project_title_name" class="form-control" placeholder="project Title">
                </div>
                <div class="form-group form-group-half">
                    <date-pick v-model="project_data_vmodel"></date-pick>
                    <input type="hidden" v-bind:name="this.project_date_name" :value="this.date_model">
                </div>
...
export default {
    props: ['dropzone_id', 'img_hidden_id', 'img_hidden_name', 'img_ref', 'main_accordion_id', 'inner_accordion_id', 'stored_project_title', 'stored_project_date', 'stored_project_img', 'project_title_name', 'project_date_name', 'previewer_class', 'remove_uploded_image_id', 'uploaded_image_remove_id', 'date_model'],
    components: {
        vueDropzone: vue2Dropzone, DatePick
    },
    data: function () {
        return {
            project_data_vmodel: this.date_model,
            options: {
            error: {
                position: 'center',
                    timeout: 3000,
