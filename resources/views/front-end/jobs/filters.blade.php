    ...
    {!! Form::open(['url' => url('search-results'), 'method' => 'get', 'class' => 'wt-formtheme wt-formsearch']) !!}
        ...
        <div class="wt-widget wt-effectiveholder">
            ...
            <div class="wt-widgetcontent">
                <fieldset>
                    <div class="form-group">
                        <input type="text" class="form-control filter-records" placeholder="{{ trans('lang.ph_search_cats') }}">
                    ...
                    @if (!empty($categories))
                        <div class="wt-checkboxholder wt-verticalscrollbar">
                            @foreach ($categories as $category)
                                @php $checked = ( !empty($_GET['categories']) && in_array($category->slug, $_GET['categories'] )) ? 'checked' : ''; @endphp
                                <span class="wt-checkbox">
                                    <input id="cat-{{{ $category->slug }}}" type="checkbox" name="categories[]" value="{{{ $category->slug }}}" {{$checked}} >
        ...
        <div class="wt-widget wt-effectiveholder">
            ...
            <div class="wt-widgetcontent">
                <fieldset>
                    @if (!empty($project_lengths))
                        <div class="wt-checkboxholder wt-verticalscrollbar">
                            @foreach ($project_lengths as $key => $length)
                                @php $checked = ( !empty($_GET['project_lengths']) && in_array($key, $_GET['project_lengths'])) ? 'checked' : '' @endphp
                                <span class="wt-checkbox">
                                    <input id="length-{{{ $key }}}" type="checkbox" name="project_lengths[]" value="{{{$key}}}" {{$checked}}>
