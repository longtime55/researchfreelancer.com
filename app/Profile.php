...
class Profile extends Model
{
    ...
    protected $fillable = [
        ...
        'projects', 'saved_freelancer', 'saved_jobs', 'saved_employers', 'rating',
        'address', 'longitude', 'latitude', 'avater', 'banner', 'gender', 'tagline',
        'description', 'delete_reason', 'delete_description', 'profile_searchable',
        'profile_blocked', 'weekly_alerts', 'message_alerts', 'payout_settings',
        'certification', 'transaction_currency', 'withdraw_details', 'years_exp',
        'market_profile', 'citation_id'
    ];
    ...
    public function storeProfile($request, $user_id)
    {
        ...
        $profile->transaction_currency = filter_var($request['transaction_currency'], FILTER_SANITIZE_STRING);
        $profile->withdraw_details = filter_var($request['withd_details'], FILTER_SANITIZE_STRING);
    ...
    public function updateExperienceEducation($request, $user_id)
    {
        ...
        if ($request['certification']) {
            foreach ($request['certification'] as $key => $cert) {
                ...
                $request_certification[$count1] = $cert;
                $count1++;
            }
        }
        ...
        $certification = !empty($request['certification']) ? serialize($request_certification) : '';
        $user_profile = $this::select('id')->where('user_id', $user_id)
            ->get()->first();
        if (!empty($user_profile->id)) {
            $profile = Profile::find($user_profile->id);
    ...
    /**
     * Update transaction_currency hourly_rate withdraw_details
     *
     * @param mixed   $request Request Attributes
     * @param integer $user_id User ID
     *
     * @return json response
     */
    public function updateFreelancerPayment($request, $user_id)
    {
        $json = array();
        $transaction_currency = !empty($request['transaction_currency']) ? $request['transaction_currency'] : '';
        $hourly_rate = !empty($request['hourly_rate']) ? $request['hourly_rate'] : '';
        $withdraw_details = !empty($request['withdraw_details']) ? $request['withdraw_details'] : '';
        $user_profile = $this::select('id')->where('user_id', $user_id)
            ->get()->first();
        if (!empty($user_profile->id)) {
            $profile = Profile::find($user_profile->id);
        ...
        $profile->user()->associate($user_id);
        $profile->transaction_currency = $transaction_currency;
        $profile->hourly_rate = $hourly_rate;
        $profile->withdraw_details = $withdraw_details;
        $profile->save();
        $json['type'] = 'success';
        return $json;
    }
    ...
    /**
     * Update writer skills years_exp market_profile citation
     *
     * @param mixed   $request Request Attributes
     * @param integer $user_id User ID
     *
    ...
    public function updateSkillsSpecialization($request, $user_id)
    {
        ...
        $f_profile = $this::select('id')->where('user_id', $user_id)
            ->get()->first();
        if (!empty($f_profile->id)) {
            $profile = Profile::find($f_profile->id);
        } else {
            $profile = $this;
        }
        $profile->user()->associate($user_id);
    ...
    public function savePayoutDetail($request, $user_id)
    {
        ...
        if (!empty($request['payout_settings'])) {
            ...
            } elseif ($payout_setting['type'] == 'rave') {
                $payouts['rave_email'] = $payout_setting['rave_email'];
