...
class SiteManagement extends Model
{
    ...
    /**
     * Store rave settings in storage.
     *
     * @param string $request meta_key
     *
     * @return \Illuminate\Http\Response
     */
    public function saveRaveSettings($request)
    {
        $enable_test = $request['enable_test'];
        $rave_title = $request['rave_title'];
        $rave_titlee = trim(preg_replace("/[^a-zA-Z]/", "", filter_var($rave_title, FILTER_SANITIZE_STRING)));
        $rave_key = $request['rave_key'];
        $rave_secret = $request['rave_secret'];
        $rave_secret_hash = $request['rave_secret_hash'];
        // $rave_encryption = $request['rave_encryption'];
        $payment_settings = array();
        // trim(preg_replace("/[^0-9]/", "", filter_var($request['phone_number'], FILTER_SANITIZE_STRING)
        $payment_settings[0]['rave_title'] = !empty($rave_title) ? $rave_title : '';
        $payment_settings[0]['rave_key'] = !empty($rave_key) ? $rave_key : '';
        $payment_settings[0]['rave_secret'] = !empty($rave_secret) ? $rave_secret : '';
        $payment_settings[0]['rave_secret_hash'] = !empty($rave_secret_hash) ? $rave_secret_hash : '';
        // $payment_settings[0]['rave_encryption'] = !empty($rave_encryption) ? $rave_encryption : '';
        $payment_settings[0]['enable_test'] = !empty($enable_test) ? $enable_test : '';
        if (!empty($payment_settings)) {
            $existing_payment_settings = SiteManagement::getMetaValue('rave_settings');
            if (!empty($existing_payment_settings)) {
                DB::table('site_managements')->where('meta_key', '=', 'rave_settings')->delete();
                Helper::changeEnv(
                    [
                        'RAVE_TITLE' => "",
                        'RAVE_PUBLIC_KEY' => "",
            ...
            DB::table('site_managements')->insert(
                [
                    'meta_key' => 'rave_settings', 'meta_value' => serialize($payment_settings),
                    "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                ]
            );
            $new_payment_settings = SiteManagement::getMetaValue('rave_settings');
            if ($new_payment_settings[0]['enable_test'] === 'true') {
                Helper::changeEnv(
                    [
                        'RAVE_TITLE'=> $rave_titlee,
                        'RAVE_PUBLIC_KEY'=> $rave_key,
                        'RAVE_SECRET_KEY'=> $rave_secret,
                        'RAVE_SECRET_HASH' => $rave_secret_hash,
