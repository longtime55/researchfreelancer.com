...
class SiteManagementController extends Controller
{
    ...
    public function settings()
    {
        ...
        $stripe_key = !empty($stripe_settings[0]['stripe_key']) ? $stripe_settings[0]['stripe_key'] : '';
        $stripe_secret = !empty($stripe_settings[0]['stripe_secret']) ? $stripe_settings[0]['stripe_secret'] : '';
        $rave_settings = $this->settings::getMetaValue('rave_settings');
        $rave_title = !empty($rave_settings[0]['rave_title']) ? $rave_settings[0]['rave_title'] : '';
        $rave_key = !empty($rave_settings[0]['rave_key']) ? $rave_settings[0]['rave_key'] : '';
        $rave_secret = !empty($rave_settings[0]['rave_secret']) ? $rave_settings[0]['rave_secret'] : '';
        $rave_secret_hash = !empty($rave_settings[0]['rave_secret_hash']) ? $rave_settings[0]['rave_secret_hash'] : '';
        // $rave_encryption = !empty($rave_settings) ? $rave_settings[0]['rave_encryption'] : '';
        ...
        if (file_exists(resource_path('views/extend/back-end/admin/settings/index.blade.php'))) {
            return view(
                ...
                compact(
                    ...
                    'client_id', 'payment_password', 'existing_payment_secret', 'payment_methods',
                    'currency', 'existing_currency', 'gmap_api_key', 'min_payout',
                    'payment_gateway', 'rave_title', 'rave_key', 'rave_secret', 'rave_secret_hash',
                    'stripe_key', 'stripe_secret', 'languages', 'selected_language', 'reg_one_title',
     ...
     * @return View
     */
    public function storeRaveSettings(Request $request)
    {
        $server = Helper::worketicIsDemoSiteAjax();
        if (!empty($server)) {
            $response['type'] = 'error';
        ...
        $this->validate(
            ...
            [
                'rave_key' => 'required',
                'rave_secret' => 'required',
                'rave_secret_hash' => 'required',
                // 'rave_encryption' => 'required',
            ]
        );
        $json = array();
        if (!empty($request)) {
            $default_settings = $this->settings->saveRaveSettings($request);
            if ($default_settings == "success") {
                $json['type'] = 'success';
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.settings_saved');
    ...
    public function getSitePaymentOption()
    {
        ...
        if (!empty($rave_settings[0]['enable_test'])) {
            $json['enable_test'] = $rave_settings[0]['enable_test'];
        } else {
            $json['enable_test'] = 'false';
        }
        return $json;
    }
