        ...
        if (!empty($login_user) && !empty($type)) {
            ...
            if (!empty($users) && $users->count() > 0) {
                foreach ($users as $key => $user) {
                    ...
                    $certifications = !empty(unserialize($user_object->profile->certification)) ? unserialize($user_object->profile->certification) : array();
                    $freelancer_certifications  = array();
                    if (!empty($certifications)) {
                        foreach ($certifications as $cert_key => $certification) {
                            $freelancer_certifications[$cert_key]['title'] = !empty($certification['degree_title']) ? $certification['degree_title'] : '';
                            $freelancer_certifications[$cert_key]['institute'] = !empty($certification['institute_title']) ? $certification['institute_title'] : '';
                            $freelancer_certifications[$cert_key]['startdate'] = !empty($certification['start_date']) ? url($certification['start_date']) : '';
                            $freelancer_certifications[$cert_key]['enddate'] = !empty($certification['end_date']) ? url($certification['end_date']) : '';
                            $freelancer_certifications[$cert_key]['description'] = !empty($certification['description']) ? url($certification['description']) : '';
                        }
                        $json[$key]['_certifications'] = $freelancer_certifications;
                    }
    ...
    public function getJobs(Request $request)
    {
        ...
        if (!empty($user) && $user->count() > 0) {
            foreach ($jobs as $key => $job) {
                ...
                $json[$key]['link'] = url('project/' . $job->slug);
            ...
            if ($json['type'] == "success") {
                ...
                if (!empty($email_settings) && !empty($email_settings[0]['email'])) {
                    ...
                    if (!empty($send_freelancer_offer->id)) {
                        ...
                        $email_params['project_link'] = url('project/' . $job->slug);
